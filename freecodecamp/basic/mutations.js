...
function mutation(arr) {
  const [firstString, secondString] = arr.map(string => string.toLowerCase().split(''));
  const includedLetters = secondString.filter(letter => firstString.includes(letter));
  return includedLetters.length === secondString.length;
}
